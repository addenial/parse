...
def get_host_data(root):
    ...
    for host in hosts:
        ...
        #try:
            #
            #current-<osmatch name
            #name,accuracy

            #how about..
            #osclass
            #ip,osfamily,vendor,osgen,type,os match name,accuracy
            #,,,~, Linux,Linux,2.6.x,general purpose,Linux 2.6.32,100

            #os_element = host.findall('os')
            #os_name = os_element[0].findall('osmatch')[0].attrib['name']

            #os_family = os_element[0].findall('osmatch')[0][0].attrib['osfamily']
            #os_vendor = os_element[0].findall('osmatch')[0][0].attrib['vendor']
            #os_gen = os_element[0].findall('osmatch')[0][0].attrib['osgen']
            #os_type = os_element[0].findall('osmatch')[0][0].attrib['type']
        ...
        try:
            ...
            for port in ports:
                ...
                try:
                    svversion = port.findall('service')[0].attrib['version']
                except (IndexError, KeyError):             
                    svversion = ''
                
                try:
                    eextrainfo =  port.findall('service')[0].attrib['extrainfo']
                except (IndexError, KeyError):       
                    eextrainfo = ''
                   
                try:
                    oostype = port.findall('service')[0].attrib['ostype']
                except (IndexError, KeyError):  
                    oostype = ''
                ...
                #port_data.extend((ip_address, os_family, os_vendor, os_gen, os_type, os_match_name, os_accuracy))
              
        
                #port_data.extend((ip_address, port_id, proto, service, product, svversion, eextrainfo, oostype, host_name  ))
                
                #######print(port_id + "," ,end='') << good but need uniques from this
                #print(port_id + ",")
...
def parse_to_csv(data):
    ...
    if not os.path.isfile(csv_name):
        ...
        #ip_address, port_id, proto, service, product, svversion, eextrainfo, oostype, host_name 
...
def print_data(data):
    ...
    for item in data:
        ...
        eee = ' '.join(item).replace(" ", "")
        #print(eee)
        #print(uniqqqpo)
        unipo.append(eee)
        
        if eee not in uniqqqpo:
            #print(eee)
            uniqqqpo.append(eee)
        #output.add(item)
        #if item not in unipo:
        #    unipo.append(item)
        #    print(item , )
        #print(item,end='')
        #continue
    ...
    #print(unipo)
    #print(uniqqqpo)
    #uniqooo = sorted(set(unipo))
    #
    #print(unipo)
    #print(' '.join(unipo) + "," , end='')
    sevven = str(','.join(unipo) + "," )
    
    for line in sevven.splitlines():
        #print(line + " yii")
        chunks = line.split(',')
        for ee in chunks:
            if ("." not in ee):
                if ee:
                    ...
                    fd = os.open("f1____.log", os.O_RDWR|os.O_APPEND|os.O_CREAT )
                    b = str.encode(ee + "\n")
                    os.write(fd,b)
                    os.close(fd)
                #aypo.append(ee)
    ...
    #print(aypo)   
    #eighht = sevven.replace(" ", ",".rstrip())
    #print(eighht)
    
    ##print(eighht)
    #uniq_values = np.unique(uniqqqpo)
    #print(uniq_values)
    #uniqqq = np.unique(yayann)
    #print(uniqqq)
    #uniqqpo = set(unipo)
    #print(unipo)
    #sss = list(data)
    ...
    for filename in args.filename:
        ...
        if(filename == '*' ):
            ...
            for filenameeach in os.listdir(cwd):
                ...
                if args.print_all:
                    ...
                    #if os.path.exists("f1____.log"):
                    #    os.remove("f1____.log")
        ...
        if args.ip_addresses:
            ...
            for addr in addrs:
                print(addr)
        if args.print_all:
            #lol duping this function when using wildcard '*'
            
            #using temp file... remove if alredy exists
            #if os.path.exists("f1____.log"):
            #    os.remove("f1____.log")
            
            #os.open("f1____.log", os.O_RDWR
            #os delte f1___log
            #"f1____.log"
            print_data(data)
            
            #take f1__log file, sort, uniq, and print--
            #print("tw")
            #delete 
            #if os.path.exists("f1____.log"):
            #    os.remove("f1____.log")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-f", "--filename",
                        ...
                        help="Specify a file containing the output of an nmap "
                             "scan in xml format. \n Use -f * to parse all .xml in current directory")
    ...
    if not args.filename:
        ...
        print("\n[-] Please specify an input file to parse. "
              "Use -f * to parse all .xml in current directory\n")
        exit()
    if not args.ip_addresses and not args.print_all:
        parser.print_help()
        print("\n[-] Please choose an output option. Use -ip, or -p\n")
        exit()
    #csv_name = args.csv
    main()

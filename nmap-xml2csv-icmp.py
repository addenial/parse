...
__description__ = """Parses the XML output from an nmap scan. The user
                  ...
                  already exists.
                  """
#many mods to add icmp parsing, finding active subnets in /24, 
...
'''
        ...
        #dira = str(ip_address)
       # port_data.extend((ip_address))
        #icmp_data.extend((dira))
        #icmp_data.extend((ip_address))
        
        #port_data.extend((ip_address, os_family, os_vendor, os_gen, os_type))

        # Add the port data to the host data
        #host_data.append((icmp_data))
        
        #host_data.append((ip_address, ''))
        host_data.append(ip_address)
...
def parse_to_csv(data):
    ...
    for item in data:
        ...
        #csv_writer.writerow([u' '.join(item).encode('utf8').strip()])
        #csv_writer.writerow([item.encode('utf8').strip()])
...
def subs_data(data):
    """Figues out unique /24 from active"""
    subzz = []
    subz24z = []
    ...
    for item in data:     
        ...
        subs1 = octets[0],octets[1],octets[2]
        
        #print(octets[0],octets[1],octets[2])
        #print(octets[3])
        #a yissss
        ...
        yzzsubs = '.'.join(subs1)
        subzz.append(yzzsubs)
        
    
    #print(subzz)
    #need to only keep uniques from this array
    uniq_subzz = set(subzz)
    
    #print("y---")
    
    #print(uniq_subzz)
    #for yiii in subzz  
    #append /24 to end of each from uniq_subzz    
    for ploop in uniq_subzz:
        print(ploop + ".0/24")    
        subz24z.append(ploop + ".0/24")
    return subz24z
...
if __name__ == '__main__':
    ...
    parser.add_argument("-subs", "--subnets",
                        help="Display a list of active /24 subnets",
                        action="store_true")    
    ...
    if not args.filename:
        ...
        exit()
    if not args.ip_addresses and not args.csv and not args.subnets and not args.print_all:
        parser.print_help()
        print("\n[-] Please choose an output option. Use -csv, -ip, -subs, or -p\n")
        exit()
    csv_name = args.csv
    main()
